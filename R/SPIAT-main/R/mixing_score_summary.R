...
mixing_score_summary<-function(formatted_data, reference_celltype = "Immune", target_celltype = "Tumor",
  ...
  for (i in reference_celltype) {
    ...
    for (j in target_celltype) {
      if (i == j) {
        df <- rbind(df[, df.cols], c(i, j, nrow(reference_cells), 
                                     nrow(target_cells), NA, NA, NA, NA))
      }
      tryCatch({
        target_cells <- formatted_data[formatted_data[,feature_colname] == j, ]
        if (nrow(reference_cells) == 0) {
          print(paste("There are no unique reference cells of specified celltype", 
                      i, "for target cell", j))
        }
        if (nrow(target_cells) == 0) {
          print(paste("There are no unique target cells of specified celltype", 
                      j, "for reference cell", i))
        }
        reference_cell_cords <- reference_cells[, c("Cell.X.Position", 
                                                    "Cell.Y.Position")]
        target_cell_cords <- target_cells[, c("Cell.X.Position", 
                                              "Cell.Y.Position")]
        reference_target_result <- dbscan::frNN(target_cell_cords, 
        ...
        if (reference_reference_interactions != 0) {
          ...
          normalised_mixing_score <- 2 * mixing_score * 
            (nrow(reference_cells) - 1)/nrow(target_cells)
        }else {
          normalised_mixing_score <- mixing_score <- NA
          print(paste("There are no reference to reference interactions for", 
                      j, "in the specified radius, cannot calculate mixing score"))
        }
        df <- rbind(df[, df.cols], c(i, j, nrow(reference_cells), 
                                     nrow(target_cells), reference_target_interactions, 
                                     reference_reference_interactions, mixing_score, 
                                     normalised_mixing_score))
      }, error = function(e) {
      })
    }
  }
  df[, 3:8] <- vapply(df[, 3:8], as.numeric, numeric(nrow(df[, 
                                                             3:8])))
  df <- df[-1, ]
  return(df)
}
