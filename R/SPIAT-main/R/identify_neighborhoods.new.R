...
#' @export
identify_neighborhoods.new<-function(sce_object, method = "hierarchical", 
  ...
  if (method == "hierarchical"){
    ...
    # formatted_data.del
    # sim_close.del <- - apcluster::negDistMat(formatted_data.del[,c("Cell.X.Position", "Cell.Y.Position")])
    ...
    big_clusters<-big_clusters[is.na(big_clusters)==F]
    ...
    cluster_ids<-cluster_ids[is.na(cluster_ids)==F]
  ...
  q <- q + geom_point(aes(color = Cluster,size=Cluster), size = 1)
  # q <- q + #geom_text(data = label_location, aes(x = Xpos, y = Ypos, label = Cluster),size=3)
  q <- q + scale_color_manual(values=cluster_colours)
  if(dim(cells_not_in_clusters)[1]!=0){
    q <- q + geom_point(data = cells_not_in_clusters,  colour = "black",size = 0.5)
  }
